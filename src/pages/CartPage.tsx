...
const CartPage = () => {
	const { state, removeFromCart, checkout } = useContext(CartContext) as CartContextType;
	console.log(state);
	return (
		<>
				...
				<tbody>
					{state.products.map((product: CartItem, index: number) => (
						<tr key={product.product._id}>
							...
							<td>
								<button onClick={() => removeFromCart(String(product.product._id))} className="btn btn-danger">
									Xoa
								</button>
							</td>
						</tr>
